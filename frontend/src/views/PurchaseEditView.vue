  ...
  <div class="page-view">
    <div class="page-header">
      <div class="page-header-left">
        <h1 class="page-title">Приход</h1>
        <p class="page-subtitle">Детали документа поступления товаров</p>
      </div>
    </div>
    <el-card v-if="doc" shadow="never">
      ...
      <div class="info">
        <div class="info-item">
          ...
          <div class="info-value">{{ doc.supplier?.name }}</div>
        </div>
        <div class="info-item">
      ...
      <div class="doc-actions">
        ...
        <el-button @click="$router.push('/purchases')">Назад</el-button>
      </div>
    </el-card>
  </div>
</template>
...
const post = async () => {
  try { await purchasesApi.post(route.params.id); ElMessage.success('Проведено'); load() }
  catch (e){ ElMessage.error(e.response?.data?.detail || e.response?.data?.message || 'Ошибка проведения') }
}
const cancel = async () => {
  try { await purchasesApi.cancel(route.params.id); ElMessage.success('Отменено'); load() }
