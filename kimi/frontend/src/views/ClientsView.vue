  ...
  <div class="page-view">
    <div class="page-header">
      <div class="page-header-left">
        ...
        <p class="page-subtitle">Справочник клиентов</p>
      </div>
    ...
    <!-- Form -->
    <el-dialog v-model="dialogVisible" :title="isEdit?'Редактировать':'Новый клиент'" width="480">
...
const load = async () => {
  loading.value = true
  const res = await referencesApi.clients({ page: page.value, per_page: perPage.value, search: search.value || undefined })
...
const save = async () => {
  try {
    if (isEdit.value) await referencesApi.updateClient(form.id, form)
    else await referencesApi.createClient(form)
...
const remove = async (id) => {
  try { await ElMessageBox.confirm('Удалить?', 'Подтверждение'); await referencesApi.removeClient(id); ElMessage.success('Удалено'); load() }
