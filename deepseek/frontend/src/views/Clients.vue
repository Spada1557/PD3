<template>
  <div class="card">
    ...
    <el-dialog v-model="showForm" :title="editing?'Редактировать клиента':'Новый клиент'" width="560px" @closed="resetForm" :close-on-click-modal="false">
...
const form=reactive({name:'',inn:'',contact_person:'',phone:'',email:'',address:''})
async function load(){loading.value=true;try{const{data}=await api.get('/clients',{params:{page:page.value,per_page:perPage.value,search:search.value}});items.value=data.data;total.value=data.total}catch(e){}finally{loading.value=false}}
...
function resetForm(){Object.assign(form,{name:'',inn:'',contact_person:'',phone:'',email:'',address:''})}
async function save(){try{if(editing.value){await api.put(`/clients/${editingId.value}`,form)}else{await api.post('/clients',form)}showForm.value=false;ElMessage.success('Сохранено');load()}catch(e){ElMessage.error(e.response?.data?.detail?.message||'')}}
async function delItem(id){try{await api.delete(`/clients/${id}`);ElMessage.success('Деактивирован');load()}catch(e){ElMessage.error(e.response?.data?.detail?.message||'')}}
onMounted(load)
</script>