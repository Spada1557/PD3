  ...
  <div>
    <div class="card">
      <div class="table-actions">
        ...
        <el-select v-model="filterClient" placeholder="Все клиенты" clearable style="width:260px" filterable @change="load">
      ...
      <el-table :data="items" v-loading="loading" highlight-current-row @row-click="openDetail" style="width:100%;cursor:pointer">
        ...
        <el-table-column label="Клиент" min-width="250" show-overflow-tooltip>
        ...
        <el-table-column label="Прибыль" width="160" align="right">
          <template #default="{row}">
            <div style="display:flex;align-items:center;gap:4px;justify-content:flex-end;white-space:nowrap">
              <svg v-if="row.total_profit > 0" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--success)" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="18 15 12 9 6 15"/></svg>
              <svg v-else-if="row.total_profit < 0" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--danger)" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>
              <span :class="row.total_profit >= 0 ? 'positive' : 'negative'" style="font-size:14px">{{ formatCurrency(row.total_profit) }}</span>
            </div>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="showForm" :title="readOnly ? `Продажа ${readOnly.number}` : (editing ? 'Редактирование продажи' : 'Новая продажа')" width="900px" @closed="resetForm" :close-on-click-modal="false" class="doc-dialog">
      <el-form :model="form" label-width="110px">
        <div class="form-row" style="margin-bottom:4px">
          <el-form-item label="Клиент">
            <el-select v-model="form.client_id" style="width:100%" filterable :disabled="!!readOnly" :teleported="false">
              <el-option v-for="c in clients" :key="c.id" :label="c.name" :value="c.id" />
            </el-select>
          </el-form-item>
...
const items = ref([]), clients = ref([]), products = ref([])
const loading = ref(false), saving = ref(false), page = ref(1), perPage = ref(10), total = ref(0)
const filterStatus = ref(null), filterClient = ref(null)
const showForm = ref(false), editing = ref(false), readOnly = ref(null)
const form = reactive({ client_id: null, date: new Date().toISOString(), comment: '', items: [{ product_id: null, quantity: 1, price: 0, amount: 0 }] })

function statusLabel(s) {
  return s === 'posted' ? 'Проведён' : s === 'draft' ? 'Черновик' : 'Отменён'
}
...
async function loadRefs() {
  try {
    const [c, p] = await Promise.all([api.get('/clients', { params: { per_page: 100 } }), api.get('/products', { params: { per_page: 100 } })])
    clients.value = c.data.data; products.value = p.data.data
...
async function openDetail(row) {
  try {
    ...
    form.client_id = data.client_id; form.date = data.date; form.comment = data.comment || ''
...
function resetForm() { editing.value = false; readOnly.value = null; Object.assign(form, { client_id: null, date: new Date().toISOString(), comment: '', items: [{ product_id: null, quantity: 1, price: 0, amount: 0 }] }) }
...
function onProductSelect(idx) {
  ...
  if (item.product_id) {
    ...
    if (product) { item.price = product.price; calcAmount(idx) }
  }
}
async function saveDoc() {
  saving.value = true
  const payload = { client_id: form.client_id, date: form.date, comment: form.comment, items: form.items.filter(i => i.product_id).map(i => ({ product_id: i.product_id, quantity: i.quantity, price: i.price, amount: Math.round(i.quantity * i.price * 100) / 100 })) }
  try {
    if (editing.value && readOnly.value) { await api.put(`/sales/${readOnly.value.id}`, payload) }
...
async function postDoc(id) {
  try { await api.post(`/sales/${id}/post`); ElMessage.success('Продажа проведена'); showForm.value = false; load() }
  catch (e) { ElMessage.error(e.response?.data?.detail?.message || 'Ошибка') }
}
async function cancelDoc(id) {
  try { await api.post(`/sales/${id}/cancel`); ElMessage.success('Проведение отменено'); showForm.value = false; load() }
  catch (e) { ElMessage.error(e.response?.data?.detail?.message || 'Ошибка') }
}
