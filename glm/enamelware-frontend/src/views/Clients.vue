<template>
  <div>
    <div class="toolbar">
      <el-input v-model="search" placeholder="Поиск клиента" prefix-icon="Search" clearable style="width:300px" @input="debounceSearch" />
    ...
    <div class="data-table">
      <el-table :data="clients" stripe v-loading="loading">
        ...
        <el-table-column prop="contact_person" label="Контактное лицо" width="180" />
        <el-table-column prop="phone" label="Телефон" width="150" />
        <el-table-column prop="email" label="Email" width="200" />
        <el-table-column prop="is_active" label="Статус" width="100">
          <template #default="{ row }">
            <el-tag :type="row.is_active ? 'success' : 'danger'" size="small">{{ row.is_active ? 'Активен' : 'Неактивен' }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="" width="120" fixed="right">
          <template #default="{ row }">
            ...
            <el-button link type="danger" @click="deleteClient(row)" style="cursor:pointer">Удал.</el-button>
          </template>
        </el-table-column>
      </el-table>
      <div style="display:flex;justify-content:flex-end;margin-top:16px">
        <el-pagination v-model:current-page="page" v-model:page-size="perPage" :page-sizes="[10,25,50]" :total="total" layout="total, sizes, prev, pager, next" @current-change="loadClients" @size-change="loadClients" />
      </div>
    </div>

    <el-dialog v-model="showDialog" :title="editItem ? 'Редактировать клиента' : 'Новый клиент'" width="500">
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="saveClient" :loading="saving" style="cursor:pointer">Сохранить</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
const form = ref({ name: '', inn: '', contact_person: '', phone: '', email: '', address: '', comment: '' })

let debounceTimer = null
function debounceSearch() { clearTimeout(debounceTimer); debounceTimer = setTimeout(loadClients, 300) }

function resetForm() {
  form.value = { name: '', inn: '', contact_person: '', phone: '', email: '', address: '', comment: '' }
}
...
async function deleteClient(row) {
  await ElMessageBox.confirm(`Удалить клиента "${row.name}"?`, 'Подтверждение')
  try { await api.delete(`/clients/${row.id}`); await loadClients(); ElMessage.success('Удалено') }
  catch (e) { ElMessage.error(e.response?.data?.detail?.message || 'Ошибка') }
}
